...
When you run `php artisan boost:install` or `php artisan boost:update` in a Sail project, the MCP server will be configured automatically.

**For most IDEs** (Cursor, Claude Code, VS Code):
...
**For PhpStorm** (uses docker exec for better compatibility):
...
**Important Notes:**
- The detection is automatic and transparent. If Sail is not detected, Boost will use the standard `php artisan boost:mcp` configuration.
- Your existing MCP servers are **always preserved**. When running `boost:install` or `boost:update`, Boost will only add or update the `laravel-boost` entry without removing your other configured MCP servers.

**Example:** If you have custom MCP servers configured:
...
After running `php artisan boost:install`, your configuration will become:
...
Your custom server remains untouched!
