...
test('written data is correct for brand new file', function (string $configKey, array $servers, string $expectedJson) {
    ...
    expect($result)->toBe(true)
        ->and($simpleContents)->toEqual($expectedJson);
...
test('updates existing plain JSON file using simple method', function () {
    ...
    expect($decoded)->toHaveKey('existing')
        ->and($decoded)->toHaveKey('other')
        ->and($decoded)->toHaveKey('nested.key')
        ->and($decoded)->toHaveKey('servers.new-server')
        ->and($decoded['servers']['new-server']['command'])->toBe('npm');
    // From fixture
});
...
test('adds to existing mcpServers in plain JSON', function () {
    ...
    expect($decoded)->toHaveKey('mcpServers.existing-server')
        ->and($decoded)->toHaveKey('mcpServers.boost')
        ->and($decoded['mcpServers']['boost']['command'])->toBe('php'); // Original preserved
    // New server added
});
...
test('preserves complex JSON5 features that VS Code supports', function () {
    ...
    expect($result)->toBeTrue()
        ->and($writtenContent)->toContain('"test"') // New server added
        ->and($writtenContent)->toContain('// Here are comments within my JSON') // Preserve block comments
        ->and($writtenContent)->toContain("// I'm trailing") // Preserve inline comments
        ->and($writtenContent)->toContain('// Ooo, pretty cool') // Preserve comments in arrays
        ->and($writtenContent)->toContain('MYSQL_HOST'); // Preserve complex nested structure
});
...
test('detects plain JSON with comments inside strings as safe', function () {
    ...
    expect($decoded)->toHaveKey('exampleCode') // Original preserved
     ->and($decoded)->toHaveKey('mcpServers.new-server') // New server added
    ->and($decoded['exampleCode'])->toContain('// here is the example code'); // Comment in string preserved
});
...
test('injects new configKey when it does not exist', function () {
    ...
    expect($result)->toBeTrue()
        ->and($writtenContent)->toContain('"mcpServers"')
        ->and($writtenContent)->toContain('"boost"')
        ->and($writtenContent)->toContain('"php"')
        ->and($writtenContent)->toContain('// No mcpServers key at all'); // Preserve existing comments
});
...
test('injects into existing configKey preserving JSON5 features', function () {
    ...
    expect($result)->toBeTrue()
        ...
        ->and($writtenContent)->toContain('mysql') // Existing server preserved
        ->and($writtenContent)->toContain('laravel-boost') // Existing server preserved
        ->and($writtenContent)->toContain('// Here are comments within my JSON') // Comments preserved
        ->and($writtenContent)->toContain('// Ooo, pretty cool'); // Inline comments preserved
});
...
test('injecting twice into existing JSON 5 doesn\'t cause duplicates', function () {
    ...
    expect($result)->toBeTrue()
        ->and($boostCounts)->toBe(1)
        ->and($capturedContent)->toContain('"boost"') // New server added
        ->and($capturedContent)->toContain('mysql') // Existing server preserved
        ->and($capturedContent)->toContain('laravel-boost') // Existing server preserved
        ->and($capturedContent)->toContain('// Here are comments within my JSON') // Comments preserved
        ->and($capturedContent)->toContain('// Ooo, pretty cool'); // Inline comments preserved
...
test('injects into empty configKey object', function () {
    ...
    expect($result)->toBeTrue()
        ->and($writtenContent)->toContain('"boost"') // New server added
        ->and($writtenContent)->toContain('// Empty mcpServers object') // Comments preserved
        ->and($writtenContent)->toContain('test_input'); // Existing content preserved
});
