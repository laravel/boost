...
test('name returns omp', function (): void {
    $agent = new Omp($this->strategyFactory);

    expect($agent->name())->toBe('omp');
});

test('displayName returns Oh My Pi', function (): void {
    $agent = new Omp($this->strategyFactory);
...
test('skillsPath returns .omp/skills by default', function (): void {
    $agent = new Omp($this->strategyFactory);

    expect($agent->skillsPath())->toBe('.omp/skills');
});

test('mcpConfigPath returns .omp/mcp.json by default', function (): void {
    $agent = new Omp($this->strategyFactory);

    expect($agent->mcpConfigPath())->toBe('.omp/mcp.json');
});

test('projectDetectionConfig detects via .omp directory and mcp.json', function (): void {
    $agent = new Omp($this->strategyFactory);
...
test('systemDetectionConfig detects omp on macOS and linux', function (Platform $platform): void {
    $agent = new Omp($this->strategyFactory);
...
test('systemDetectionConfig detects omp on windows', function (): void {
    $agent = new Omp($this->strategyFactory);
...
test('mcpServerConfig returns omp stdio config', function (): void {
    $agent = new Omp($this->strategyFactory);
...
test('installMcp writes omp project mcp config', function (): void {
    $mcpPath = '.omp/mcp.json';
